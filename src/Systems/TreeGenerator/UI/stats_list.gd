...
func _create_stat_item(index: int, stat: PointStat) -> void:
	...
	stat_item.add_child(stat_select)
	...
	stat_item.add_child(mode_select)
	...
	stat_item.add_child(value_edit)
	...
	stat_item.add_child(remove_button)

func _on_add_stat_pressed() -> void:
	if current_point:
		var new_stat := PointStat.new(Stat.STAT.DEX, StatMode.STAT_MODE.FLAT, 0)
